        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {pdfs.map((pdf) => {
            ...
            return (
              <div
                key={pdf.id}
                className={`bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex gap-3 ${locked ? "opacity-70" : ""}`}
                style={{ borderLeftColor: locked ? "#e5e7eb" : tokens.border, borderLeftWidth: 3 }}
              >
                <div
                  className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0"
                  style={locked ? { backgroundColor: "#f3f4f6" } : { backgroundColor: tokens.bg }}
                >
                  {locked ? (
                ...
                <div className="min-w-0 flex-1 flex flex-col gap-1">
                  ...
                  <h3 className={`font-semibold text-sm leading-snug line-clamp-2 ${locked ? "text-gray-400" : "text-[#2D1B69]"}`}>
                    {pdf.title}
                  </h3>
                  {!locked && pdf.fileSize != null && (

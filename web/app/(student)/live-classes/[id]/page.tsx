...
export default async function LiveClassDetailPage({
  ...
  const isUpcoming  = cls.liveStatus === "UPCOMING";
  ...
  const dateLabel = formatSessionDateTime(
    cls.sessionDate,
    cls.startTime,
    cls.endTime
  );
      ...
      <div className={`bg-white rounded-2xl border overflow-hidden ${isLive ? "border-red-200 shadow-lg shadow-red-50" : "border-gray-100"}`}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold ${statusColors[cls.liveStatus] ?? "bg-gray-100 text-gray-600"} ${isLive ? "animate-pulse" : ""}`}>
              {isLive && <span className="w-1.5 h-1.5 rounded-full bg-white inline-block" />}
              {statusLabel[cls.liveStatus] ?? cls.liveStatus}
            </span>
            {cls.platform === "ZOOM" && (
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-blue-600 text-white leading-none">Zoom</span>
            )}
            {cls.platform === "YOUTUBE_LIVE" && (
              <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-red-600 text-white leading-none">YouTube Live</span>
            )}
          </div>
          ...
          <div className="pt-2 space-y-3">
            ...
            {/* LIVE NOW but join not yet available */}
            {isLive && !cls.canJoin && (
            ...
            {isLocked && (
              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-start gap-2 text-sm text-amber-800">
                ...
                This class is not yet accessible. It will unlock automatically at the scheduled time.
              </div>
            )}
          </div>
        </div>

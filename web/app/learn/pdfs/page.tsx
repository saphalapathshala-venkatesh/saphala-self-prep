...
export default async function PdfsPage() {
  const [user, pdfs] = await Promise.all([
    getCurrentUser(),
    getPublishedPdfs(),
  ]);
    ...
    <main className="min-h-screen flex flex-col bg-gray-50">
      ...
      <section className="bg-white py-10 border-b border-gray-100">
        <div className="container mx-auto px-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#6D4BCB] mb-1">
            Prep Library
          </p>
          ...
          <p className="text-gray-500 text-sm max-w-xl">
            Download and study curated PDFs for your exam preparation.
            {!isLoggedIn && (
              <span>
                {" "}
                <Link
                  ...
                  <div className="mt-auto pt-2">
                    {isLoggedIn ? (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#6D4BCB] hover:text-[#5E3FB8] transition-colors"
                      >
                        <svg
                      ...
                      <Link
                        href="/login"
                        className="text-xs text-gray-400 hover:text-[#6D4BCB] transition-colors"
                      >
                        Login to download →
                      </Link>
                    )}

...
Pricing for courses lives on the `Course` table (admin-owned, raw SQL): `isFree` (boolean), `mrp` (numeric, rupees), `sellingPrice` (numeric, rupees). The paise columns (`mrpPaise`, `sellingPricePaise`) are always NULL and must not be used. `courseDb.ts` selects `mrp` and `sellingPrice` in both `getActiveCourses()` and `getCourseWithCurriculum()`, plus LEFT JOIN LATERAL on `ProductPackage` (matching `entitlementCodes @> ARRAY[productCategory]`) to resolve `packageId` for the checkout URL. `discountPercent` is computed from `round((mrp - sellingPrice) / mrp * 100)`. Prices are formatted as `₹${value.toLocaleString("en-IN")}` — no division by 100. All student surfaces (course listing, dashboard courses, course detail, featured courses on homepage) display selling price, MRP strikethrough, discount %, and "Buy Now →" CTA pointing to `/checkout?packageId=<id>` only when `sellingPrice > 0`. Free courses (`productCategory === "FREE_DEMO"` or `isFree = true`) always show "Start Free →". Validity is shown on the course detail page from `validityType`, `validityDays`, `validityMonths`, `validUntil` columns.

...
Pricing for courses lives on the `Course` table (admin-owned, raw SQL): `isFree` (boolean), `mrp` (numeric, rupees), `sellingPrice` (numeric, rupees). The paise columns (`mrpPaise`, `sellingPricePaise`) are always NULL and must not be used. `courseDb.ts` selects `mrp`, `sellingPrice`, `packageId`, and `packagePricePaise` in both `getActiveCourses()` and `getCourseWithCurriculum()`, via LEFT JOIN LATERAL on `ProductPackage` (matching `entitlementCodes @> ARRAY[productCategory]`, selecting `id` and `pricePaise`). The **effective selling price** = `packagePricePaise / 100` when a linked package exists, otherwise `sellingPrice`. `discountPercent` is computed via `computeDiscount(mrp, effectiveSelling)`. This ensures the price shown on course cards and the course detail page always matches what the checkout page will charge. Prices are formatted as `₹${value.toLocaleString("en-IN")}` — no division by 100. All student surfaces (course catalog, dashboard courses, course detail, featured courses on homepage) display effectiveSelling as buy price, MRP as strikethrough, discount %, validity, and "Buy Now →" CTA pointing to `/checkout?packageId=<id>` only when effectiveSelling > 0. Free courses (`productCategory === "FREE_DEMO"` or `isFree = true`) always show "Start Free →". Validity is shown from `validityType`, `validityDays`, `validityMonths`, `validUntil` columns on course cards and the course detail page. `getCourseWithCurriculum` parallelizes the course header + sections queries with `Promise.all` for performance.
